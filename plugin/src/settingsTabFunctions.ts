...
export class settingsTabFunctions {
  ...
  createSystemFolder = async (newSystemFolderPath: string) => {
    try {
      ...
      if (!newSystemFolder) {
        ...
        await this.plugin.utilities.safeCreateOrModifyFile(readmePath, content);
  ...
  flowBuilder = async (
    ...
    for (let ingredient of recipeArray) {
      ...
      if (caller === "settingsTab") {
        ...
        const symbolEmpty = this.plugin.utilities.explorerDecoArray[0][0];
        const symbolFilled = this.plugin.settings.explorerDecoStyle[1];
    ...
    if (systemFolder && systemFolder instanceof TFolder) {
      ...
      await this.plugin.utilities.safeCreateOrModifyFile(
        flowFilePath,
            ...
            if (this.plugin.settings.activeRegions[flowName][leafID]) {
              ...
              if (
                ...
                if (view instanceof MarkdownView) {
                  this.plugin.utilities.restoreCursorPos(
                    flowName,
                    view,
                    leafID,
                  );
  ...
  backupFlowDefs = async () => {
    ...
    for (let flowName of Object.keys(this.plugin.settings.flows)) {
      const currentDate = this.plugin.utilities.getTimestamp();
