...
new, so not all quirks are known/fixed/worked around yet. If you find anything
not mentioned in this Readme [let me know](#12-report-a-bug--report-your-love).
Please also consider running Obsidian's own data recovery plugin or 'Edit
history' by Antonio Tejada, at least until textFlow has earned your trust.
...
9. **Convenience in a menu bar:** If you prefer buttons, textFlow has with a
   ...
   2. **Your cursor history:** textFlow saves the last 3 cursor positions for
      the last three regions of the newest 3 leaves (updated with every sync)
      and there's a button as well as a command to restore the last known cursor
      position for a leaf. Entries for leaves older than 24 hours are deleted,
      with the exception of the newest 2.
...
1. **No scrolling via fuzzy navigation on uninitialised leaves:** If you reload
   your vault and then use the fuzzy navigation modal to target a leaf that is
   open but has not yet been activated, it will scroll to its last known cursor.
2. **Necessary data duplication:** Flows are extra notes which repeat their
...
3. **Flows can NOT be safely opened in other text editors:** The non-printing
...
4. **No manual sorting:** If you use a plugin to manually sort your notes in
...
5. **Wiki-links don't work:** Right now wiki-links don't work inside of flows. I
   might fix it some day, maybe, if enough people ask for it.
6. **No extra stuff in reading mode:** textFlow is an editing tool, so there is
...
3. **Implicit size limit for flows:** Obsidian handles open notes in memory, so
   ...
   spread over several - will make the UI, as well as textFlow's various
   functions, sluggish. So maybe keep the flows on the smaller side and only
   open what you actually need. Also:
...
- **Installation:** While the plugin hasn't been released to the marketplace
  yet, you can install it using BRAT or going the manual route:
...
- **Basic idea:** textFlow copies the contents of certain notes into a new note,
  ...
  apparently.
- **Flow:** A note that has been created (compiled) from a selection of notes
  and equipped with UUIDs in order to allow for functionality like the tracking
  of the cursor position and edits as well as syncing edits back to the correct
  source note.
